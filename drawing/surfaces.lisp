...
(defun sample-surface (f u0 u1 steps-u v0 v1 steps-v)
  (let ((us (loop for i from 0 to steps-u
                  collect (+ u0 (* (- u1 u0) (/ i steps-u)))))
        (vs (loop for j from 0 to steps-v
                  collect (+ v0 (* (- v1 v0) (/ j steps-v))))))
    (loop for v in vs
          collect (loop for u in us collect (funcall f u v)))))

(defun quad-normal (p00 p10 p01)
  (norm (cross-3 (v- p10 p00) (v- p01 p00))))

(defun draw-surface-wire (grid &key name style)
  (loop for row in grid
        do (emit :polyline :points row :closed nil
...
(defun draw-surface-faces (grid &key style back-style (cull t))
...
(defun draw-surface (f &key (u0 0d0) (u1 1d0) (steps-u 20)
                           ...
                           (mode :wire) name style back-style (cull t))
  (let ((grid (sample-surface f u0 u1 steps-u v0 v1 steps-v)))
    (ecase mode
      (:wire (draw-surface-wire grid :name name :style style))
